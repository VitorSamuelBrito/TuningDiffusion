...
#-----------------------
## Initial parameters
coef_init_free = np.polyfit(energy_x, energy_y, 4) 
...
p0_pol_free = coef_init_free[::-1]  
p0_pol_dq = coef_init_dq[::-1] 
# p0_g = [A0, loc0, beta0, C0]                                      

#-----------------------
## Curve fitting
popp, pcop = curve_fit(libdiff.quartic_model, energy_x, energy_y, p0=p0_pol_free, maxfev=10000)
popt, pcot = curve_fit(libdiff.third_model, data_x, data_y, p0=p0_pol_dq, maxfev=10000)
# popg, pcog = curve_fit(
...
# plt.plot(data_x, D_itp, label='DQ fit', color='red')
# plt.legend()
...
# plt.plot(data_x, V_itp, label='VQ', color='black')
# plt.legend()
# plt.xlabel("reaction coordinate Q")
# plt.ylabel("drift")
# plt.title("Drift")
...
'- Gerar novas trajetorias com novo DQ_fit -  feito'
'- Mudar, passar a salvar DQ e FQ interpolados - feito'
'- Salvar gráficos novos interpolados - feito'
'- Salvar gráficos ajustados e aprox - feito'
'- Rodar DrDrift com as trajetorias geradas - função polinomial preferência, mais rodar com a gumbel também'
'- Rodar DrDrift com a trajetoria simulada'
'- Comparar tempos de passagem e etc'